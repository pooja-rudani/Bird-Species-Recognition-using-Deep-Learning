...
class Resnet_Model:
    ...
    def __init__(self, path_to_pretrained_model=None, map_location='cpu', num_classes=310):
        """
        ...
        self.image_train_dataset = datasets.ImageFolder(os.path.join(self.data_dir, 'train'), self.train_transform)
        self.image_valid_dataset = datasets.ImageFolder(os.path.join(self.data_dir, 'valid'), self.val_transform)
        self.image_test_dataset = datasets.ImageFolder(os.path.join(self.data_dir, 'test'), self.test_transform)
        self.train_loader = DataLoader(dataset=self.image_train_dataset, batch_size=16, shuffle=True)
        self.val_loader = DataLoader(dataset=self.image_valid_dataset, batch_size=16, shuffle=True)
        self.test_loader = DataLoader(dataset=self.image_test_dataset, batch_size=16, shuffle=True)
        if path_to_pretrained_model:
            self.model = torch.load(path_to_pretrained_model, map_location=map_location)
        else:
            self.model = self._setup_resnet(num_classes=310)
        ...
        for epoch in range(num_epochs):
            ...
            for phase in phases:
                ...
                if phase == 'val' and epoch_acc > best_acc:
                    ...
                    torch.save(model, 'trained_model_resnet50.pt')
                    epochs_no_improve = 0
